...
class Tracker:
    ...
    def __init__(self, metric, max_iou_distance=0.7, max_age=30, n_init=3, use_apperance=True):
        self.metric = metric
        self.max_iou_distance = max_iou_distance
        self.max_age = max_age
        self.n_init = n_init
        self.use_apperance = use_apperance  # ✅ NEW
    ...
    def update(self, detections):
        ...
        for track in self.tracks:
            ...
            track.features = []
        if self.use_apperance and self.metric is not None:
            self.metric.partial_fit(
                np.asarray(features), np.asarray(targets), active_targets)


    def _match(self, detections):
        if not self.use_apperance or self.metric is None:
            # IOU-only matching for all tracks
